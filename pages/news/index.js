...
const News = ({data}) => {
    ...
    useEffect(() => {
        ...
        const nextResults = isMore
            ? data.slice(currentLength + 1, currentLength + 6)
            : []
    ...
    useEffect(() => {
        const isMore = showList.length < (data.length - 1); // minus the first blog that is the large blog card on the knowledge page
        setHasMore(isMore)
    }, [showList])
