...
const PageTitle = ({type, title, subTitle = 'season 11'}) => {
    ...
    return ( <div ref={pageTitleRef} className="space-y-1 font-black text-center xl:space-y-3 2xl:space-y-4">
        ...
        <h1 ref={titleRef} className="border-text text-[44px] lg:text-headline-48 invisible hero-title 2xl:text-hero-80 xl:text-[72px] tracking-widest leading-10  lg:text-glow-strong  text-primary-500">{title}</h1>
    </div> );
}
