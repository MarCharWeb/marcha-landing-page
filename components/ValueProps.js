...
const ValueProps = () => {
    ...
    useLayoutEffect(() => {
        ...
        const gsapSelector = gsap.utils.selector(valuePropsContentRef);
        
        gsap.from(gsapSelector('.value'), {
            opacity: 0, 
            autoAlpha: 0, 
            ease: "power2.out", 
            y: 20, 
    ...
    }, [valuePropsContentRef, valuePropsContainerRef]);
    

    return ( <section ref={valuePropsContainerRef} className="relative z-[2] grid grid-cols-1 col-span-1 row-span-1  grid-row-1" >
        
        {/* Content */}
        <div className={'relative col-start-1 col-end-2 row-start-1 row-end-2 pt-16 xl:pt-20'  }>
            <PageTitle title={"WHAT'S IN IT FOR ME"}></PageTitle>  
            ...
            text={'Expose diverse examination topics in each round and have the valuable privilege to receive feedback from leading industry experts'}
            ></Value>
