const Button = ({type ='button', className, onClick, text, isPrimary = true, isGlow=false, size='small', loadingText='Loading', isLoading=false}) => {
    return ( <button 
            type={type}
            className={'font-bold text-secondary-900 rounded-full flex justify-center items-center ' + (isPrimary ? 'bg-gradient-to-r from-[#FFDA19] to-[#CC7B00]  ': 'border border-secondary-600 ') + (size == 'small' && 'px-6 py-0.5 text-body-18  ') + (isLoading ? 'opacity-50 pointer-events-none': '') + className}
            onMouseDown={onClick}>
