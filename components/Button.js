...
const Button =forwardRef( ({type ='button', className, onClick, text, isPrimary = true, isGlow=false, size='small', loadingText='Hang on...', isLoading=false}, ref) => {
    return ( <button 
            ref={ref}
            type={type}
            className={'font-bold text-secondary-900 rounded-full flex justify-center items-center  hover:text-primary-500 duration-300 ' + (isPrimary
    ? 'text-secondary-900 bg-gradient-to-r from-[#FFDA19] to-[#CC7B00]'
    : 'text-white border border-secondary-600 ') 
            + (size == 'small' ? 'px-6 py-0.5 ' : size == 'large' ? 'px-6 py-3 text-headline-26 ':'') + (isLoading ? 'opacity-50 pointer-events-none': '') 
